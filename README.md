...
│                                                                         │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐                 │
│  │ spec-writer │ → │ spec-execu- │ → │   归档      │                 │
│  │             │   │   tor       │   │   06-已归档  │                 │
│  │ 创建 plan.md│   │ 执行实现    │   └─────────────┘                 │
│  └─────────────┘   └─────────────┘                                   │
│         ↓               ↓                                               │
│    用户确认         用户确认 summary.md                                   │
│                                                                         │
│  ┌─────────────┐                                                       │
│  │spec-reviewer│  可选：用户可在任意时刻调用进行详细审查                      │
│  └─────────────┘                                                       │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │                    Obsidian 作为核心存储                        │  │
│  │  • Markdown 文档（plan.md, summary.md）                        │  │
│  │  • 双链建立文档关联                                              │  │
│  │  • Frontmatter 管理元数据                                         │  │
│  │  • Callout 突出关键信息                                           │  │
...
| `exp-search` | 经验检索 | 检索三层记忆（经验+SOP+工具记忆） |
...
├── update-001-summary.md      # 更新总结（spec-updater 创庺）
...
**手动触发**：
```bash
/exp-search <关键词>   # 检索相关经验
/exp-reflect           # 反思对话，提取可沉淀的经验
