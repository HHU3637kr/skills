...
mcp__obsidian-spec-confirm__spec_confirm(
    file_path="spec/分类目录/YYYYMMDD-HHMM-任务描述/update-xxx.md",
    doc_type="update",
    title="更新方案 - 功能名称"
)
```
...
```
mcp__obsidian-spec-confirm__spec_confirm(
    file_path="spec/分类目录/YYYYMMDD-HHMM-任务描述/update-xxx-review.md",
    doc_type="review",
    title="更新审查报告 - 功能名称"
)
```

响应处理：`action: "continue"` → 继续；`action: "modify"` → 根据 `userMessage` 调整后重新确认。
...
4. **等待用户确认**：调用 MCP `spec_confirm` 工具（节点 1）
5. **读取 execution_mode**：
...
| A13 | 等待用户确认审查报告 | 调用 MCP `spec_confirm`（节点 2），完成后不归档 |
