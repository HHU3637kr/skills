...
### 步骤 4：等待用户确认更新方案（使用 MCP 工具）

**操作**：
1. 告知用户 `update-xxx.md` 已创建
2. 使用 `spec_confirm` MCP 工具等待用户确认更新方案

**使用 MCP 工具确认**：
```python
# 等待用户确认更新方案
mcp__obsidian-spec-confirm__spec_confirm(
    file_path="spec/03-功能实现/20251231-专业评价Agent设计/update-001.md",
    doc_type="update",
    title="更新方案 - 权重验证功能"
)
```
...
**确认流程**：
1. 工具调用后，侧边栏会自动打开，显示更新方案信息和确认按钮
2. 用户在 Obsidian 中审阅 update-xxx.md
3. 审阅完成后，用户在侧边栏点击"✓ 确认"或"✗ 需要修改"
4. 收到确认响应后，根据用户反馈继续执行或修改方案

**响应处理**：
- `action: "continue"` - 用户确认更新方案，开始执行更新
- `action: "modify"` - 用户需要修改方案，根据 `userMessage` 调整后重新确认
...
1. 告知用户审查报告已生成
2. 提供审查结果摘要
3. 使用 `spec_confirm` MCP 工具等待用户确认

**使用 MCP 工具确认**：
```python
# 等待用户确认更新审查报告
mcp__obsidian-spec-confirm__spec_confirm(
    file_path="spec/03-功能实现/20251231-专业评价Agent设计/update-001-review.md",
    doc_type="review",
    title="更新审查报告 - 权重验证功能"
)
```
...
2. 用户在 Obsidian 中审阅 update-xxx-review.md
3. 审阅完成后，用户在侧边栏点击"✓ 确认"或"✗ 需要修改"
4. 收到确认响应后，根据用户反馈完成或修复问题

**响应处理**：
- `action: "continue"` - 用户确认更新无问题，更新完成
- `action: "modify"` - 用户发现问题，根据 `userMessage` 修复后重新审查
