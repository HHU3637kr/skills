...
import { ClaudeCodeAdapter, CodexAdapter } from "./agentAdapters/cliAdapters";
...
class AgentChatViewProvider implements vscode.WebviewViewProvider {
  ...
  async refresh(): Promise<void> {
    ...
    this.webviewView.webview.html = renderAgentChatHtml(spec, this.activeRole);
  }
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const adapters: AgentAdapter[] = [new ClaudeCodeAdapter(), new CodexAdapter()];
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("rkFlow.openAgentTeamCanvas", async (spec?: SpecBinding) => {
      ...
      await openAgentTeamCanvas(context, activeSpec, gitBinding, teamBus, chatProvider);
    }),
...
async function openAgentTeamCanvas(
  ...
  const panel = vscode.window.createWebviewPanel(
    ...
    const messages = await teamBus.readMessages(spec.id, "TeamLead");
  const roleChatMessages = await readPrivateRoleChat(spec);
  const roleSessions = await readRoleSessions(spec);
  panel.webview.html = renderCanvasHtml(spec, currentBranch, messages, roleChatMessages, roleSessions);
  panel.webview.onDidReceiveMessage(async message => {
    ...
    if (message.command === "roleMessage" && isAgentRole(message.role) && typeof message.body === "string") {
      await chatProvider.selectRole(message.role);
      await chatProvider.sendRoleMessage(spec, message.role, message.body, typeof message.model === "string" ? message.model : "default", payload => {
    ...
    if (message.command === "teamMessage" && typeof message.body === "string") {
      ...
      const subject = typeof message.subject === "string" && message.subject.trim() ? message.subject.trim() : "Canvas note";
      const teamMessage = await teamBus.sendMessage({
        specId: spec.id,
        from,
        to,
        type,
      ...
      panel.webview.postMessage({ type: "teamMessage", message: teamMessage });
      await chatProvider.routeTeamMessages(spec, [teamMessage], payload => {
        panel.webview.postMessage(payload);
      });
    }
...
function renderCanvasHtml(
  ...
  const agents: Array<{ role: AgentRole; engine: string; x: number; y: number }> = [
    ...
    { role: "spec-executor", engine: "Codex CLI", x: 330, y: 246 },
    { role: "spec-tester", engine: "Codex CLI", x: 582, y: 286 },
    ...
    <aside class="rolePanel">
      ...
      <form class="roleForm" id="roleComposer">
        <select id="roleModel">
          ...
          <option value="gpt-5.3-codex">Codex CLI · gpt-5.3-codex</option>
...
function renderAgentChatHtml(spec: SpecBinding | undefined, activeRole: AgentRole): string {
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
...
function defaultEngineForRole(role: AgentRole): AgentEngine {
  return role === "spec-executor" || role === "spec-tester" ? "codex-cli" : "claude-code";
}

function normalizeModelForEngine(engine: AgentEngine, model: string): string {
  if (engine === "codex-cli") {
    return model === "default" || model === "claude-default" ? "gpt-5.3-codex" : model;
  }
...
function extractExternalSessionId(event: AgentEvent, engine: AgentEngine): string | undefined {
  ...
  if (engine === "codex-cli" && raw.type === "thread.started" && typeof raw.thread_id === "string") {
