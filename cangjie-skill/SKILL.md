...
## 核心方法论: RIA-TV++
一个四阶段 + 并行提取 + 三重验证 + darwin 兼容测试的流水线。详见 `references/methodology/00-overview.md`。
...
2. 执行 `references/methodology/01-stage0-adler.md` 中的 Adler 四步 (结构 / 解释 / 批判 / 应用)。
3. 按 `references/templates/BOOK_OVERVIEW.md.template` 填充,写入 `books/<slug>/BOOK_OVERVIEW.md`。
...
|---|---|---|
| 框架提取器 | `references/extractors/framework-extractor.md` | 决策框架 / 思维模型 |
| 原则提取器 | `references/extractors/principle-extractor.md` | 原则 / 清单 / 规则 |
| 案例提取器 | `references/extractors/case-extractor.md` | 作者在书中亲自使用过的实例 |
| 反例提取器 | `references/extractors/counter-example-extractor.md` | 书中警告的失败模式 |
| 术语提取器 | `references/extractors/glossary-extractor.md` | 关键概念词典 |

每个 sub-agent 独立读书、独立提取、独立输出到 `books/<slug>/candidates/<type>.md`。

### 阶段 1.5 — 三重验证筛选
读取 `references/methodology/03-stage1.5-triple-verify.md`,对每个候选单元执行:
...
### 阶段 2 — RIA++ 构造 skill
对每个通过的单元,按 `references/templates/SKILL.md.template` 填充:
...
细则见 `references/methodology/04-stage2-ria-plus.md`。

### 阶段 3 — Zettelkasten 链接
按 `references/methodology/05-stage3-zettelkasten.md`:
1. 找出 skill 之间的引用关系 (A 依赖 B / A 对比 B / A 组合 B)
2. 在每个 SKILL.md 末尾补"相关 skills"段
3. 按 `references/templates/INDEX.md.template` 生成 `INDEX.md` (含引用图 mermaid)

### 阶段 4 — 压力测试 (darwin 兼容)
对每个 skill 按 `references/methodology/06-stage4-pressure-test.md`:
1. 设计 5–10 条测试 prompt,按 `references/templates/test-prompts.json.template` 写入 `test-prompts.json`
