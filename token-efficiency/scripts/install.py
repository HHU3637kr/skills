...
from _paths import ADAPTERS, BEHAVIOR, CONTEXT, CORE, MARKER, OUTPUT, REPO_ROOT

ALL_AGENTS = ("cursor", "claude-code", "codex", "windsurf", "hermes", "cline", "aider", "continue", "gemini", "generic")
...
def install_claude_code(project: Path, levers: Set[str], dry_run: bool, _: bool) -> List[str]:
    ...
    parts: List[str] = []
    if "1" in levers:
        parts.append("### L1 Behavior\n\n" + read_core(BEHAVIOR))
    if "3" in levers:
        parts.append("### L3 Output\n\n" + read_core(OUTPUT))
    if parts:
...
def install_codex(project: Path, levers: Set[str], dry_run: bool, merge: bool) -> List[str]:
    ...
    if "1" in levers or "3" in levers or "0" in levers:
...
def install_cline(project: Path, levers: Set[str], dry_run: bool, merge: bool) -> List[str]:
    # Cline reads .cursorrules + CLAUDE.md
...
def install_hermes(project: Path, levers: Set[str], dry_run: bool, _: bool) -> List[str]:
    actions: List[str] = []
    # Skill itself goes via install_skill.py → ~/.hermes/skills/token-efficiency/
...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument("--levers", default="1,3", help="0=context 1=behavior 3=output")
    ...
    print("token-efficiency install v0.1")
