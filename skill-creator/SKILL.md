...
### 步骤 4.6：行为验证（约束类 Skill 必做）

结构检查（frontmatter 齐全、引用文件存在）只能证明 Skill 装得下，不能证明它起作用。**面向 Agent 的文档，其正确性只能由消费方的行为来验证。**

适用范围：Skill 意在改变 Agent 的选择（纪律、约束、流程门禁）时必做。纯查阅型 Skill（API 参考、格式说明）不必做。

三条不可跳过的要求：

1. **先跑基线。** 在没有这条规则时把场景交给 Agent，记录它的实际选择。如果它自发就做对了，删掉这条规则——别为想象中的错误立规。那次违规才是 RED。
2. **每个场景至少叠加 3 种压力。** 单一压力等于没测。
3. **「表示理解规则」不算通过。** 只有实际动作算。Agent 说「你说得对，我应该先写测试」然后直接写实现，是不通过。

构造方法、七种压力类型、场景五要素、元测试三分法（遵守 / 违规 / **规避**）、按内容类型选写法 → 必读 `references/skill-pressure-testing.md`。
...
7. 约束类 Skill（意在改变 Agent 的选择）：已按步骤 4.6 跑过基线与压力场景，并记录三分法结果。结构检查通过 ≠ 规则起作用
