---
name: exp-reflect
description: 经验反思 Skill，读取当前任务的 Spec 文档（plan/summary/explore/debug 等）提取记忆并按类型分流：困境-策略对→experience/，项目理解/技术调研→knowledge/。支持用户提示词（如"记录数据流"）自动识别类型。触发场景：任务完成后（建议性）、解决困难问题后、用户要求总结经验、用户要求记录项目理解。
...
- [ ] 步骤 1：读取当前任务的 Spec 文档
      ...
      | `test-report.md` | 测试结果、发现的问题 | 经验记忆（测试发现的边界问题） |
