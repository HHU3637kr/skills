...
> 把本 Skill 当成一个只读的检索循环单元：明确读什么、能动什么、怎么算完成、什么时候停、什么时候交还给人。

| 项 | 本 Skill 的约定 |
|----|----------------|
| 输入 | 检索关键词或场景描述；五层记忆索引与详情、Auto Memory（只读） |
| 权限 | 只读（Read/Glob/Grep）；不写任何文件、不修改记忆、不替用户外推结论 |
| 验证 | 按记忆类型分组展示匹配项，命中项可追溯到具体文件路径或记忆 ID |
| 停止 | 展示完匹配结果即停止；无匹配时给出建议而非编造记忆 |
| 升级 | 检索不到且问题需要解决时，提示用户走 exp-reflect 沉淀新经验，由用户决定下一步 |
