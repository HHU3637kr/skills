...
R&K Flow VS Code Extension 的 MVP 目标是在不重做 IDE、不重做完整 Coding Agent 的前提下，把现有 Spec 工作流可视化，并通过 Claude Code CLI 承载 AgentRole 的实际对话执行。

核心设计是：VS Code 负责项目、文件树、编辑器和扩展宿主；R&K Flow 扩展负责 Spec Explorer、AgentTeam Canvas、Role Chat、TeamBus、Git 分支绑定和 ClaudeCodeAdapter。Codex CLI 暂不作为 MVP 支持项暴露，后续新 CLI 通过 Adapter 扩展。
...
- `AgentTeam Canvas`：Webview Panel，展示当前 Spec 的角色节点和 Git 分支状态；点击角色后切换右侧 Role Chat。
...
- `Agent Adapter Layer`：当前只注册 ClaudeCodeAdapter，封装 Claude Code CLI 的检测、启动、resume 和 stream-json 事件转换。
...
5. 用户发送消息后，扩展使用 ClaudeCodeAdapter 启动或 resume 对应 CLI 会话。
...
- `src/agentAdapters/cliAdapters.ts`：Claude Code CLI 适配。
