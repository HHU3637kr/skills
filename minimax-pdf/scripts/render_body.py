...
def register_cjk_fonts(tokens: dict) -> None:
    """Register a CJK-capable font so Chinese/Japanese/Korean text is not garbled.

    ReportLab's built-in Helvetica/Times have no CJK glyphs. On Windows we use
    Microsoft YaHei (msyh.ttc). macOS/Linux can be extended later (e.g. Noto).
    """
    if tokens.get("_cjk_fonts_registered"):
        return

    windir = os.environ.get("WINDIR", r"C:\Windows")
    fonts_dir = os.path.join(windir, "Fonts")
    msyh = os.path.join(fonts_dir, "msyh.ttc")
    msyhbd = os.path.join(fonts_dir, "msyhbd.ttc")

    if platform.system() == "Windows" and os.path.isfile(msyh):
        try:
            pdfmetrics.registerFont(TTFont("YaHeiCJK", msyh, subfontIndex=0))
            bold_name = "YaHeiCJK-Bold"
            if os.path.isfile(msyhbd):
                pdfmetrics.registerFont(TTFont(bold_name, msyhbd, subfontIndex=0))
            else:
                bold_name = "YaHeiCJK"

            tokens["font_display_rl"] = "YaHeiCJK"
            tokens["font_body_rl"] = "YaHeiCJK"
            tokens["font_body_b_rl"] = bold_name
            # Monospace code blocks: Courier has no CJK; use same family for mixed content
            tokens["font_mono_rl"] = "YaHeiCJK"
            tokens["_cjk_fonts_registered"] = True
        except Exception:
            pass
...
def _add_bibliography(story: list, item: dict, ctx: dict):
    ...
    for ref in item.get("items", []):
        ...
        story.append(BibliographyItem(
            ...
            ctx["styles"]["bib"],
            ctx["dark"],
            label_font=ctx["tokens"]["font_body_b_rl"],
        ))
