...
def _md_inline(text: str) -> str:
    ...
    # Inline code FIRST — protects snake_case (e.g. memory_storage) from
    # _italic_ matching across underscores before backticks are applied.
    text = re.sub(r'`(.+?)`', r'<font name="Courier">\1</font>', text)
    # Bold: **text** or __text__
    text = re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', text)
    text = re.sub(r'__(.+?)__',     r'<b>\1</b>', text)
    # Italic: *text* or _text_ (underscore: not between identifier chars — avoids snake_case)
